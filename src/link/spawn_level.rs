...
impl Plugin for SpawnPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(SpawnState::Settling), spawn_level)
            ...
            .add_plugins(
                StateInspectorPlugin::<SpawnState>::default().run_if(
                    input_toggle_active(false, KeyCode::Grave)
                        .and_then(not(in_state(SpawnState::None))),
                ),
            )
            .insert_resource(SettleTimer(Timer::from_seconds(0.25, TimerMode::Once)))
...
fn on_enter_link(mut spawn_state: ResMut<NextState<SpawnState>>) {
    spawn_state.set(SpawnState::Settling);
}

fn on_exit_link(mut spawn_state: ResMut<NextState<SpawnState>>) {
    spawn_state.set(SpawnState::None);
}
...
fn spawn_walls(
    ...
    commands.spawn((
        ...
        Position(Vec2::new(0.0, TOP - OUTER_WALL_THICKNESS / 2.0)),
        MaterialMesh2dBundle {
    ...
    println!(
        "Spawned inlet block at y={}",
        TOP - OUTER_WALL_THICKNESS / 2.0
    );
    ...
    commands.spawn((
        ...
        Position(Vec2::new(0.0, BOTTOM + OUTER_WALL_THICKNESS / 2.0)),
        MaterialMesh2dBundle {
