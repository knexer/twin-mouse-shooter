...
impl Plugin for PlayerPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(MischiefPlugin)
            ...
            .add_plugins(
                StateInspectorPlugin::<AttachState>::default().run_if(
                    input_toggle_active(false, KeyCode::Grave)
                        .and_then(not(in_state(AttachState::None))),
                ),
            )
            .add_systems(
                Update,
            ...
            .add_systems(
                Update,
                move_cursors.after(poll_events).run_if(
                    in_state(CurrentGame::Link).and_then(input_toggle_active(true, KeyCode::Grave)),
                ),
            )
            .add_systems(
                FixedUpdate,
                apply_cursor_force
                    ...
                    .run_if(in_state(CurrentGame::Link)),
            )
            .add_systems(OnEnter(CurrentGame::Link), on_enter_link)
            .add_systems(OnExit(CurrentGame::Link), on_exit_link);
    }
}

fn on_enter_link(mut state: ResMut<NextState<AttachState>>) {
    state.set(AttachState::Waiting);
}

fn on_exit_link(mut state: ResMut<NextState<AttachState>>) {
    state.set(AttachState::None);
}
...
fn attach_cursors(
    ...
    let left_cursor_device = match left_cursors.get_single() {
        Ok(cursor) => cursor.0,
        Err(_) => return,
    };
    let right_cursor_device = match right_cursors.get_single() {
        Ok(cursor) => cursor.0,
        Err(_) => return,
    };
